(function (global, factory) {
  ...
  var gsap$1,
    ...
    _getGSAP$1 = function _getGSAP() {
      return (
        gsap$1 || (_windowExists() && (gsap$1 = window.gsap) && gsap$1.registerPlugin && gsap$1)
      );
    },
    ...
    _getViewportDimension = function _getViewportDimension(dimensionProperty) {
      return (
        (dimensionProperty === 'Height' ? _100vh : _win$1['inner' + dimensionProperty]) ||
    ...
    _clearScrollMemory = function _clearScrollMemory(scrollRestoration, force) {
      ...
      _isString(scrollRestoration) &&
        (_win$1.history.scrollRestoration = _scrollRestoration = scrollRestoration);
    },
    _refreshingAll,
    _refreshID = 0,
    _queueRefreshID,
    ...
    _parsePosition = function _parsePosition(
      ...
      if (marker) {
        ...
        _positionMarker(
          ...
          (isStart && position > 20) ||
            (!isStart &&
              (useFixedPosition ? Math.max(_body$1[p1], _docEl$1[p1]) : marker.parentNode[p1]) <=
                position + 1)
        );
    ...
    _proto.init = function init(vars, animation) {
      ...
      if (snap) {
        ...
        _scrollers.forEach(function (o) {
          return (
            _isFunction(o) &&
            o.target === (isViewport ? _doc$1.scrollingElement || _docEl$1 : scroller) &&
            (o.smooth = false)
          );
        });
            ...
            if (
              ...
              var progress = (scroll - start) / change,
                ...
                velocity = refreshedRecently
                  ? 0
                  : ((totalProgress - snap2) / (_getTime$1() - _time2)) * 1000 || 0,
      ...
      if (markers) {
        ...
        containerAnimation &&
          (caMarkerSetter = gsap$1.quickSetter([markerStart, markerEnd], direction.a, _px));
      ...
      self.refresh = function (soft, force, position, pinOffset) {
        ...
        if (pin && !(_clampingMax && self.end >= _maxScroll(scroller, direction))) {
          ...
          if (!max && end > 1) {
            ...
            if (
              isViewport &&
              _getComputedStyle(_body$1)['overflow' + direction.a.toUpperCase()] !== 'scroll'
            ) {
      ...
      self.update = function (reset, recordVelocity, forceFake) {
        ...
        if (anticipatePin && pin && !_refreshing && !_startup$1 && _lastScrollTime) {
          if (
            !clipped &&
            start < scroll + ((scroll - scroll2) / (_getTime$1() - _time2)) * anticipatePin
          ) {
            clipped = 0.0001;
          } else if (
            clipped === 1 &&
            end > scroll + ((scroll - scroll2) / (_getTime$1() - _time2)) * anticipatePin
          ) {
            clipped = 0.9999;
          }
        }
    ...
    _inputObserver = function _inputObserver(target, type, inputs, nested) {
      return Observer.create({
        ...
        onEnable: function onEnable() {
          return (
            inputs && _addListener$1(_doc$1, Observer.eventTypes[0], _captureInputs, false, true)
          );
        },
