...
let gsap: any,
  ...
  _elements = (targets: SplitTextTarget): HTMLElement[] =>
    _toArray(targets).filter(e => e instanceof HTMLElement) as HTMLElement[],
  _emptyArray: string[] = [],
  ...
  _insertNodeBefore = (newChild: Node | string, parent: Element, existingChild: Node): Node =>
    parent.insertBefore(
      typeof newChild === 'string' ? document.createTextNode(newChild) : (newChild as Node),
      existingChild
    ),
  ...
  _splitWordsAndCharsRecursively = (
    ...
    if (typeof wordDelimiter === 'object') {
      wordDelimSplitter =
        (wordDelimiter as WordDelimiterConfig).delimiter || (wordDelimiter as string | RegExp);
          ...
          if (!reduceWhiteSpace && wordText === '') {
            ...
            if (
              ...
              ignoredPreviousSibling &&
                j === 1 &&
                !startsWithSpace &&
                curWordEl.insertBefore(ignoredPreviousSibling as Element, curWordEl.firstChild);
            }
        ...
        if (splitLines) {
          let nodes: Node[] = _toArray(element.childNodes),
            ...
            allBounds: BoundingRect[] = nodes.map(n =>
              n.nodeType === 1 ? (n as Element).getBoundingClientRect() : _emptyBounds
