...
const TopographicBackground = () => {
  ...
  useEffect(() => {
    ...
    const generateField = () => {
      ...
      fieldRef.current = Array.from({ length: cols }, (_, x) =>
        Array.from({ length: rows }, (_, y) => (simplex.noise2D(x * scale, y * scale) + 1) / 2)
      );
    };
          ...
          for (let y = 0; y < rows - 1; y++) {
            ...
            const checkAndAddHighlight = (p1, p2) => {
              ...
              const dist = Math.sqrt(
                Math.pow(midX - mousePos.current.x, 2) + Math.pow(midY - mousePos.current.y, 2)
              );
            ...
            switch (state) {
              case 1:
              case 14:
                ctx.moveTo(p_l.x, p_l.y);
                ctx.lineTo(p_b.x, p_b.y);
                checkAndAddHighlight(p_l, p_b);
                break;
              case 2:
              case 13:
                ctx.moveTo(p_b.x, p_b.y);
                ctx.lineTo(p_r.x, p_r.y);
                checkAndAddHighlight(p_b, p_r);
                break;
              case 3:
              case 12:
                ctx.moveTo(p_l.x, p_l.y);
                ctx.lineTo(p_r.x, p_r.y);
                checkAndAddHighlight(p_l, p_r);
                break;
              case 4:
              case 11:
                ctx.moveTo(p_t.x, p_t.y);
                ctx.lineTo(p_r.x, p_r.y);
                checkAndAddHighlight(p_t, p_r);
                break;
              case 5:
              case 10:
                ...
                if (center_avg > threshold) {
                  ...
                  ctx.moveTo(p_t.x, p_t.y);
                  ctx.lineTo(p_l.x, p_l.y);
                  checkAndAddHighlight(p_t, p_l);
                  ctx.moveTo(p_b.x, p_b.y);
                  ctx.lineTo(p_r.x, p_r.y);
                  checkAndAddHighlight(p_b, p_r);
                }
  ...
  return (
    <canvas
      ref={canvasRef}
      style={{ position: 'relative', top: 0, left: 0, zIndex: -1, backgroundColor: '#E9E3DF' }}
    />
  );
};
...
export default function HomeEnd() {
  ...
  return (
    <div className="relative h-full text-[#222222] border-t-2 border-[#222222] overflow-hidden">
      ...
      <div className="absolute z-10 top-0 left-0 p-2 md:p-3">
        ...
        <h1 className="text-[10vw] md:text-[6vw] lg:text-[5vw] font-thin text-[#222222] lowercase tracking-tighter leading-none select-none drop-shadow-lg">
          THE{' '}
          <span className="text-[15vw] md:text-[8vw] lg:text-[6vw] font-bold font-serif italic">
            art{' '}
          </span>{' '}
          i.e.{' '}
          <span className="text-[15vw] md:text-[8vw] lg:text-[6vw] font-thin font-serif italic">
            nurture,
          </span>
          <br></br>
          <span className="text-[18vw] md:text-[10vw] lg:text-[8vw] font-black">
            EXPERIENCE DESIGN.
          </span>
        </h1>
      </div>
      ...
      <div className="absolute z-10 bottom-0 left-0 right-0 p-4 md:p-8 flex justify-between items-end">
        <div className="text-left">
          <p className="text-2xl md:text-4xl lg:text-5xl text-[#222222] max-w-md drop-shadow-md px-1 mb-18 md:mb-8">
            feel at home, explore.
          </p>
        </div>
        <div>
          <button
            onClick={() => router.push('/services')}
            className="px-2 py-1.5 bg-black text-white font-black mb-18 text-[9vw] md:text-[3vw] lg:text-[5vw] mb-4 md:mb-8 shadow-lg hover:bg-[white] hover:text-black focus:outline-none opacity-60 focus:ring-2 focus:ring-red-400 focus:ring-opacity-75 transition-transform transform hover:scale-105"
          >
            /services
          </button>
        </div>
