...
export default function App() {
  ...
  const topBarRef = useRef(null); // Ref for the browser's top bar
  const webglCanvasRef = useRef(null);
  const justTextRef = useRef(null); // Ref for the "JUST" text
  const expressTextRef = useRef(null); // Ref for the "EXPRESS" text
  const namasteTextRef = useRef(null); // Ref for the 2D "Namaste" text
  const threeJsStuff = useRef({}).current; // Ref to hold three.js variables
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      textureLoader.load('/assets/media/heroBG.jpg', 
        (texture) => { // Success
      ...
      let mouseX = 0;
      const onMouseMove = (event) => { mouseX = (event.clientX / window.innerWidth) * 2 - 1; };
  ...
  return (
    <>
      <div 
        ...
        className="relative h-screen w-full overflow-hidden bg-[#18230F] flex items-center justify-center"
        style={{ perspective: '1000px', fontFamily: 'Inter, sans-serif' }}
      >
      ...
      <div className="relative w-full h-full px] bg-[#222222] overflow-hidden">
 <TextScroll />
