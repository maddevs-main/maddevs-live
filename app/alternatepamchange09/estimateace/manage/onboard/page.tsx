...
export default function Page() {
  ...
  const [view, setView] = useState<
    'default' | 'all' | 'pending' | 'approved' | 'rejected' | 'completed'
  >('default');
  ...
  const fetchOnboards = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(data.error || 'Failed to fetch onboard entries');
      }

      // Fix: Use 'meetings' key instead of 'onboards' to match backend response
  ...
  const handleApproveConfirm = async (approved: boolean) => {
    ...
    try {
      const res = await fetch(`/api/onboard/${approvalDialog.onboardId}/approve`, {
        method: 'PATCH',
        headers: getAuthHeaders(),
        body: JSON.stringify({
          approved,
  ...
  const handleDone = async (id: string) => {
    setUpdating(id + 'done');
    try {
      const res = await fetch(`/api/onboard/${id}/done`, {
        method: 'PATCH',
        headers: getAuthHeaders(),
          ...
          {data.map(o => (
            ...
            >
              ...
              <td style={{ border: '1px solid #eee', padding: 10 }}>
                {o.approved === true ? 'Yes' : o.approved === false ? 'No' : 'None'}
              </td>
              {showDone && (
                <td style={{ border: '1px solid #eee', padding: 10 }}>
                  {o.done === true ? 'Yes' : 'No'}
                ...
                {showApprove && (
                  <button
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      opacity:
                        updating === o._id + 'true' || updating === o._id + 'false' ? 0.5 : 1,
                    }}
                  >
                    Approve
                  </button>
                ...
                {showDone && o.done !== true && (
                  <button
                    ...
                    style={{
                      ...
                      border: 'none',
                      cursor: 'pointer',
                      opacity: updating === o._id + 'done' ? 0.5 : 1,
                    }}
                  >
                    Done
                  </button>
  ...
  return (
    <>
      ...
      >
        <a
          href="/alternatepamchange09/estimateace"
          style={{ fontSize: 20, color: '#0074d9', textDecoration: 'underline', marginBottom: 8 }}
        >
          DASH
        </a>
        ...
        <div style={{ marginBottom: 24, display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'default' ? '#222' : '#eee',
              color: view === 'default' ? '#fff' : '#222',
              border: 'none',
              fontWeight: 600,
              cursor: 'pointer',
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'all' ? '#222' : '#eee',
              color: view === 'all' ? '#fff' : '#222',
              border: 'none',
              fontWeight: 600,
              cursor: 'pointer',
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'pending' ? '#222' : '#eee',
              color: view === 'pending' ? '#fff' : '#222',
              border: 'none',
              fontWeight: 600,
              cursor: 'pointer',
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'approved' ? '#222' : '#eee',
              color: view === 'approved' ? '#fff' : '#222',
              border: 'none',
              fontWeight: 600,
              cursor: 'pointer',
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'rejected' ? '#222' : '#eee',
              color: view === 'rejected' ? '#fff' : '#222',
              border: 'none',
              fontWeight: 600,
              cursor: 'pointer',
            }}
          ...
          <button
            ...
            style={{
              padding: '8px 18px',
              borderRadius: 4,
              background: view === 'completed' ? '#222' : '#eee',
